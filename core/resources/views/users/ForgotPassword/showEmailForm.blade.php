    ...
    <div class="col-md-8 col-md-offset-2">
      ...
      <form id="sendResetPassMailForm" action="{{route('users.sendResetPassMail')}}" class="" method="post">
        ...
        <div class="form-group">
          ...
          <input placeholder="Votre adresse e-mail" class="form-control" type="email" name="resetEmail" value="" style="height:60px;">
          @if ($errors->has('resetEmail'))
            <p class="text-danger">{{$errors->first('resetEmail')}}</p>
          @endif
        </div>
        <div class="form-group text-center">
          <input class="btn btn-success" type="submit" name="" value="Envoyer mail de rénitialisation du mot de passe">
        </div>
      </form>
    </div>
  </div>
